 ...
 * @desc
 */
@ToolIntent(items = @ToolIntentItem(value = "vidu_t2v", description = "提供基于Vidu(唯度)的文生视频能力，擅长二次元动漫风格"))
@Conditional(DashScopeOpsController.DashScopeCondition.class)
@ConditionalOnExpression("${ai.tools.vidu-t2v.enable:true}")
@Component
@Data
@Slf4j
@NoArgsConstructor
@AllArgsConstructor
@Tools
public class DashScopeVideoViduText2VideoTools implements AsyncTaskResolver {
    public static final String TASK_TYPE = "dashscope_vidu_text2video";

    private static DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss-SSS");

    public static final String DEFAULT_MODEL = "vidu/viduq3-pro_text2video";
    public static final String DEFAULT_MODEL_I2V = "vidu/viduq3-pro_img2video";
    ...
    @Value("${ai.tools.vidu-t2v.model:vidu/viduq3-pro_text2video}")
    protected String model = DEFAULT_MODEL;

    @Value("${ai.tools.vidu-t2v.model-i2v:vidu/viduq3-pro_img2video}")
    ...
    public AsyncTaskMessage text_to_video_vidu(
        ...
        if (reference_image_url != null && !reference_image_url.isEmpty()) {
            ...
            firstFrameMedia.put("type", "image");
