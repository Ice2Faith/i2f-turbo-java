...
import i2f.springboot.ops.dashscope.controller.DashScopeOpsImageViduText2ImageController;
...
import i2f.springboot.ops.dashscope.data.DashScopeImageViduText2ImageOperateDto;
...
@AllArgsConstructor
@Tools
public class DashScopeImageViduText2ImageTools implements AsyncTaskResolver {
    public static final String TASK_TYPE = "dashscope_vidu_text2image";
    ...
    @Value("${ai.tools.vidu-t2i.model:vidu/vidu-image-pro_reference2image}")
    ...
    public AsyncTaskMessage text_to_image_vidu(
        ...
        if (imageViduText2ImageController == null) {
        ...
        DashScopeImageViduText2ImageOperateDto dto = new DashScopeImageViduText2ImageOperateDto();
        dto.setModelName(modelName);
        ...
        dto.getParameters().put("n", 1);
        dto.getParameters().put("size", "1920*1088");
        dto.getParameters().put("watermark", false);
        if (portrait_mode) {
            dto.getParameters().put("size", "1088*1920");
        }
        ...
        taskItem.setDescription("Vidu文生图-图片生成");
    ...
    public AsyncTaskItem resolve(AsyncTaskItem item, OpenAiMeta meta) throws Exception {
        ...
        if ("PENDING".equals(taskStatus)) {
            ...
            for (String url : downloadUrlList) {
                String virtualFileName = "vidu-image-" + (files.size() + 1) + "-" + (TIME_FORMATTER.format(LocalDateTime.now())) + ".png";
                try {
