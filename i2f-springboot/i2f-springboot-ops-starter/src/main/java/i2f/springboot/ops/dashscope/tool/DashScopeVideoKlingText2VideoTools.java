...
import i2f.springboot.ops.dashscope.controller.DashScopeOpsVideoKlingController;
    ...
    public AsyncTaskItem resolve(AsyncTaskItem item, OpenAiMeta meta) throws Exception {
        ...
        if ("PENDING".equals(taskStatus)) {
            ...
            for (String url : downloadUrlList) {
                String virtualFileName = "kling-video-" + (files.size() + 1) + "-" + (TIME_FORMATTER.format(LocalDateTime.now())) + ".mp4";
                try {
