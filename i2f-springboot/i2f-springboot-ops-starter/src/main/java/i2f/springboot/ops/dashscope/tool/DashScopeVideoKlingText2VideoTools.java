...
import i2f.springboot.ops.dashscope.controller.*;
import i2f.springboot.ops.dashscope.data.*;
...
@AllArgsConstructor
@Tools
public class DashScopeVideoKlingText2VideoTools implements AsyncTaskResolver {
    public static final String TASK_TYPE = "dashscope_kling_text2video";

    private static DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss-SSS");

    public static final String DEFAULT_MODEL = "kling/kling-v3-omni-video-generation";
    public static final String DEFAULT_MODEL_I2V = "kling/kling-v3-omni-video-generation";

    @Autowired(required = false)
    private DashScopeOpsVideoKlingController videoKingController;
    ...
    @Value("${ai.tools.kling-t2v.model:kling/kling-v3-omni-video-generation}")
    protected String model = DEFAULT_MODEL;

    @Value("${ai.tools.kling-t2v.model-i2v:kling/kling-v3-omni-video-generation}")
    ...
    public AsyncTaskMessage text_to_video_kling(
        ...
        DashScopeVideoKlingOperateDto dto = new DashScopeVideoKlingOperateDto();
        dto.setModelName(modelName);
        ...
        if (reference_image_url != null && !reference_image_url.isEmpty()) {
            dto.getParameters().put("mode","std");
        ...
        dashScopeMeta.setApiKey(meta.getApiKey());
        dto.setMeta(dashScopeMeta);
        String taskId = videoKingController.videoKling(dto);

        AsyncTaskItem taskItem = new AsyncTaskItem();
        taskItem.setDescription("可灵-视频生成");
    ...
    public AsyncTaskItem resolve(AsyncTaskItem item, OpenAiMeta meta) throws Exception {
        ...
        if ("PENDING".equals(taskStatus)) {
            ...
            for (String url : downloadUrlList) {
                String virtualFileName = "kling-video-" + (files.size() + 1) + "-" + (TIME_FORMATTER.format(LocalDateTime.now())) + ".png";
                try {
