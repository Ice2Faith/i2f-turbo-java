...
/**
 * 将自闭合标签转换为标准的开闭标签对
 * 例如: <chapter-section v-for="ch in spec.chapters" :key="ch.id" />
 * 转换为: <chapter-section v-for="ch in spec.chapters" :key="ch.id" ></chapter-section>
 */
function convertSelfClosingToOpenClose(html) {
    // 匹配自闭合标签的正则表达式
    // 分组说明:
    // $1: 标签名（含可能的连字符等）
    // $2: 标签内的属性内容
    const selfClosingRegex = /<([a-zA-Z][a-zA-Z0-9-]*)((?:\s+[^>]*?)?)\s*\/>/g;

    return html.replace(selfClosingRegex, (match, tagName, attrs) => {
        // 构造开标签
...
Vue2Loader.parseHtmlDom = function (html) {
    html=convertTagName(html)
    html=convertSelfClosingToOpenClose(html)
