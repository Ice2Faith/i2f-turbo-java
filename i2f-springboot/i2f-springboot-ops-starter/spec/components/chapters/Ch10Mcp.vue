<template>
    <div>
        ...
        <BodyText size="13.5px">动态工具虽能按需装载，但 LLM 仍需从零开始「翻目录」——对上百个工具提供商的场景，三步发现本身就需要消耗额外的推理轮次。意图识别方案在 LRU 动态工具的基础上引入<b>前置过滤</b>：利用 <code>@ToolIntent</code> 注解为每个工具标注意图标签，再由 LLM 根据用户问题<b>先推断意图</b>，将推断出的意图标签对应的工具<b>优先插入 LRU 队列头部</b>，让 LLM 在后续推理中<b>更大概率直接命中所需工具</b>。触发条件：仅当末消息是用户消息时始终触发；若末消息是 assistant/tool（中间过程），则保留 <b>30% 概率随机触发</b>，与系统提示词重注入策略一致。</BodyText>
