...
@Data
@NoArgsConstructor
public class SwlFilter extends OncePerHttpServletFilter {
    public static final String SWL_REQUEST_RAW_HEADER_ATTR_KEY = "SWL_RAW_HEADER";
    public static final String SWL_REQUEST_HEADER_ATTR_KEY = "SWL_HEADER";
    public static final String SWL_REQUEST_DECRYPT_ATTR_KEY = "SWL_DECRYPTED";
    public static final String SWL_REQUEST_DECRYPT_EXCEPTION_ATTR_KEY = "SWL_EXCEPTION";
    public static final String SWL_RESPONSE_ENCRYPT_ATTR_KEY = "SWL_ENCRYPTED";
    protected SwlTransfer transfer;
    protected SwlWebConfig config;
    protected IJsonSerializer jsonSerializer;

    public SwlFilter(SwlTransfer transfer, SwlWebConfig config, IJsonSerializer jsonSerializer) {
    ...
    public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        SwlCtrl ctrl = parseCtrl(request, response);
        ...
        // 判断是否已经被解密过
        Object decrypted = request.getAttribute(SWL_REQUEST_DECRYPT_ATTR_KEY);
        ...
        if (ctrl.isIn() && !Boolean.TRUE.equals(decrypted)) {
            try {
                ...
                // 保存原始请求头
                nextRequest.setAttribute(SWL_REQUEST_RAW_HEADER_ATTR_KEY, data.getHeader());

                // 接受数据
                SwlData receiveData = transfer.receive(clientIp, data);

                // 保存解密后的头
                nextRequest.setAttribute(SWL_REQUEST_HEADER_ATTR_KEY, receiveData.getHeader());
                ...
                nextRequest.setAttribute(SWL_REQUEST_DECRYPT_EXCEPTION_ATTR_KEY, e);
            }

            // 设置为已被解密
            nextRequest.setAttribute(SWL_REQUEST_DECRYPT_ATTR_KEY, true);
        ...
        // 如果已被加密，则不再处理
        Object encrypted = request.getAttribute(SWL_RESPONSE_ENCRYPT_ATTR_KEY);
        if (Boolean.TRUE.equals(encrypted)) {
            return;
        }

        request.setAttribute(SWL_RESPONSE_ENCRYPT_ATTR_KEY, true);
