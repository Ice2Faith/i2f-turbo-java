...
public class TinyScriptParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // ( KEY_AS | KEY_CAST  | KEY_IS | KEY_INSTANCE_OF | KEY_TYPE_OF) express
  public static boolean castInstanceOfRightPart(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "castInstanceOfRightPart")) return false;
    boolean r;
    Marker m = enter_section_(b, l, _NONE_, CAST_INSTANCE_OF_RIGHT_PART, "<cast instance of right part>");
    r = castInstanceOfRightPart_0(b, l + 1);
    r = r && express(b, l + 1);
    exit_section_(b, l, m, r, false, null);
    return r;
  }

  // KEY_AS | KEY_CAST  | KEY_IS | KEY_INSTANCE_OF | KEY_TYPE_OF
  private static boolean castInstanceOfRightPart_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "castInstanceOfRightPart_0")) return false;
    boolean r;
    r = consumeToken(b, KEY_AS);
    if (!r) r = consumeToken(b, KEY_CAST);
    if (!r) r = consumeToken(b, KEY_IS);
    if (!r) r = consumeToken(b, KEY_INSTANCE_OF);
    if (!r) r = consumeToken(b, KEY_TYPE_OF);
    return r;
  }
  ...
  /* ********************************************************** */
  // KEY_IN | KEY_NOT_IN| OP_GTE | KEY_GTE | OP_LTE | KEY_LTE | OP_NE | KEY_NE | OP_NEQ | KEY_NEQ | OP_EQ | KEY_EQ | OP_GT | KEY_GT | OP_LT | KEY_LT
  public static boolean compareOperatorPart(PsiBuilder b, int l) {
    ...
    r = consumeToken(b, KEY_IN);
    if (!r) r = consumeToken(b, KEY_NOT_IN);
    if (!r) r = consumeToken(b, OP_GTE);
    if (!r) r = consumeToken(b, KEY_GTE);
    if (!r) r = consumeToken(b, OP_LTE);
    if (!r) r = consumeToken(b, KEY_LTE);
    if (!r) r = consumeToken(b, OP_NE);
    if (!r) r = consumeToken(b, KEY_NE);
    if (!r) r = consumeToken(b, OP_NEQ);
    if (!r) r = consumeToken(b, KEY_NEQ);
    if (!r) r = consumeToken(b, OP_EQ);
    if (!r) r = consumeToken(b, KEY_EQ);
    if (!r) r = consumeToken(b, OP_GT);
    if (!r) r = consumeToken(b, KEY_GT);
    if (!r) r = consumeToken(b, OP_LT);
    if (!r) r = consumeToken(b, KEY_LT);
    exit_section_(b, l, m, r, false, null);
    return r;
  }
  ...
  /* ********************************************************** */
  // KEY_FUNC NAMING TERM_PAREN_L parameterList? TERM_PAREN_R scriptBlock
  public static boolean declareFunction(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "declareFunction")) return false;
    if (!nextTokenIs(b, KEY_FUNC)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeTokens(b, 0, KEY_FUNC, NAMING, TERM_PAREN_L);
    r = r && declareFunction_3(b, l + 1);
    r = r && consumeToken(b, TERM_PAREN_R);
    r = r && scriptBlock(b, l + 1);
    exit_section_(b, m, DECLARE_FUNCTION, r);
    return r;
  }

  // parameterList?
  private static boolean declareFunction_3(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "declareFunction_3")) return false;
    parameterList(b, l + 1);
    return true;
  }
  ...
  /* ********************************************************** */
  // (ROUTE_NAMING|NAMING|extractExpress|staticEnumValue) (OP_ASSIGN_IFNULL|OP_ASSIGN_NOTNULL|OP_ASSIGN_ADD|OP_ASSIGN_SUB|OP_ASSIGN_MUL|OP_ASSIGN_DIV|OP_ASSIGN_MOD|OP_ASSIGN) express
  ...
  // OP_ASSIGN_IFNULL|OP_ASSIGN_NOTNULL|OP_ASSIGN_ADD|OP_ASSIGN_SUB|OP_ASSIGN_MUL|OP_ASSIGN_DIV|OP_ASSIGN_MOD|OP_ASSIGN
  private static boolean equalValue_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "equalValue_1")) return false;
    boolean r;
    r = consumeToken(b, OP_ASSIGN_IFNULL);
  ...
  public static boolean logicalLinkOperatorPart(PsiBuilder b, int l) {
    ...
    if (!r) r = consumeToken(b, KEY_AND);
    if (!r) r = consumeToken(b, OP_OR);
    if (!r) r = consumeToken(b, KEY_OR);
    exit_section_(b, l, m, r, false, null);
    return r;
  }
  ...
  //     | castInstanceOfRightPart // 类型转换/判断类型表达式
  ...
  public static boolean operatorSegment(PsiBuilder b, int l) {
    ...
    r = squareQuoteRightPart(b, l + 1);
    if (!r) r = castInstanceOfRightPart(b, l + 1);
  ...
  private static boolean parameterList_1_0(PsiBuilder b, int l) {
    ...
    boolean r;
    Marker m = enter_section_(b);
    r = consumeTokens(b, 0, TERM_COMMA, NAMING);
    exit_section_(b, m, null, r);
    return r;
  }
