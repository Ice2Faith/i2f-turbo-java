 ...
 * @desc
 */
public class PandocCmdRagFileReader implements RagFileReader {
    public static final PandocCmdRagFileReader INSTANCE = new PandocCmdRagFileReader();
    public static final Set<String> SUFFIXES = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
            ".doc", ".docx",
            ".xls", ".xlsx",
            ".ppt", ".pptx",
            ".pdf",
            ".odt", ".rtf", ".epub", ".fb2", ".opml",
            ".tex", ".latex", ".dbk"
    )));
        ...
        try {
            ...
            if (OsUtil.isWindows()) {
                cmdOutput = OsUtil.execCmd(true, TimeUnit.MINUTES.toMillis(3),
                        new String[]{"cmd", "/c", "pandoc", file.getName(), "-o", tmpFile.getName()},
                        null, parentFile, null
                );
            } else {
                cmdOutput = OsUtil.execCmd(true, TimeUnit.MINUTES.toMillis(3),
                        new String[]{"sh", "-c", "pandoc", file.getName(), "-o", tmpFile.getName()},
                        null, parentFile, null
                );
            }
            if (!tmpFile.exists()) {
