 ...
 * @desc
 */
public class EasyOcrCmdRagFileReader implements RagFileReader {
    public static final EasyOcrCmdRagFileReader INSTANCE = new EasyOcrCmdRagFileReader();
        ...
        try {
            ...
            if (OsUtil.isWindows()) {
                cmdOutput = OsUtil.execCmd(true, TimeUnit.MINUTES.toMillis(3),
                        new String[]{"cmd", "/c", "python", scriptFile.getName(), file.getName(), "-o", tmpFile.getName()},
                        null, parentFile, null
                );
            } else {
                cmdOutput = OsUtil.execCmd(true, TimeUnit.MINUTES.toMillis(3),
                        new String[]{"sh", "-c", "python", scriptFile.getName(), file.getName(), "-o", tmpFile.getName()},
                        null, parentFile, null
                );
            }
            if (!tmpFile.exists()) {
