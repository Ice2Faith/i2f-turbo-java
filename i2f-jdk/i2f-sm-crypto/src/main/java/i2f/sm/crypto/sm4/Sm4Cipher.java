package i2f.sm.crypto.sm4;

import com.sun.istack.internal.Nullable;
import i2f.codec.bytes.charset.CharsetStringByteCodec;
import i2f.codec.bytes.raw.HexStringByteCodec;
import i2f.sm.crypto.SmException;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/**
 * @author Ice2Faith
 * @date 2025/8/13 17:30
 */
public class Sm4Cipher {
    public static enum CryptFlag {
        DECRYPT,
        ENCRYPT
    }

    public static enum Padding {
        PKCS_7, // pkcs#7
        PKCS_5, // pkcs#5
    }

    public static enum Mode {
        ECB, // default
        CBC
    }

    public static final int ROUND = 32;
    public static final int BLOCK = 16;

    public static final int[] Sbox = {
            
    };

    public static final int[] CK = {
            0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
            0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
            0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
            0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
            0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
            0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
            0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
            0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
    };

    /**
     * 16 进制串转字节数组
     */
    public static byte[] hexToArray(String str) {
        return HexStringByteCodec.INSTANCE.decode(str);
    }

    /**
     * 字节数组转 16 进制串
     */
    public static String ArrayToHex(byte[] arr) {
        return HexStringByteCodec.INSTANCE.encode(arr);
    }

    /**
     * utf8 串转字节数组
     */
    public static byte[] utf8ToArray(String str) {
        return CharsetStringByteCodec.UTF8.decode(str);
    }

    /**
     * 字节数组转 utf8 串
     */
    public static String arrayToUtf8(byte[] arr) {
        return CharsetStringByteCodec.UTF8.encode(arr);
    }

    /**
     * 32 比特循环左移
     */
    public static int rotl(int x, int n) {
        int s = n & 31;
        return (x << s) | (x >>> (32 - s));
    }

    /**
     * 非线性变换
     */
    public static int byteSub(int a) {
        return (Sbox[a >>> 24 & 0xFF] & 0xFF) << 24 |
                (Sbox[a >>> 16 & 0xFF] & 0xFF) << 16 |
                (Sbox[a >>> 8 & 0xFF] & 0xFF) << 8 |
                (Sbox[a & 0xFF] & 0xFF);
    }

    /**
     * 线性变换，加密/解密用
     */
    public static int l1(int b) {
        return b ^ rotl(b, 2) ^ rotl(b, 10) ^ rotl(b, 18) ^ rotl(b, 24);
    }

    /**
     * 线性变换，生成轮密钥用
     */
    public static int l2(int b) {
        return b ^ rotl(b, 13) ^ rotl(b, 23);
    }

    /**
     * 以一组 128 比特进行加密/解密操作
     */
    public static void sms4Crypt(byte[] input, byte[] output, int[] roundKey) {
        int[] x = new int[4];


        // 字节数组转成字数组（此处 1 字 = 32 比特）
        int[] tmp = new int[4];
        for (int i = 0; i < 4; i++) {
            tmp[0] = input[4 * i] & 0x0ff;
            tmp[1] = input[4 * i + 1] & 0x0ff;
            tmp[2] = input[4 * i + 2] & 0x0ff;
            tmp[3] = input[4 * i + 3] & 0x0ff;
            x[i] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3];
        }

        // x[i + 4] = x[i] ^ l1(byteSub(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ roundKey[i]))
        for (int r = 0, mid; r < 32; r += 4) {
            mid = x[1] ^ x[2] ^ x[3] ^ roundKey[r + 0];
            x[0] ^= l1(byteSub(mid)); // x[4]

            mid = x[2] ^ x[3] ^ x[0] ^ roundKey[r + 1];
            x[1] ^= l1(byteSub(mid)); // x[5]

            mid = x[3] ^ x[0] ^ x[1] ^ roundKey[r + 2];
            x[2] ^= l1(byteSub(mid)); // x[6]

            mid = x[0] ^ x[1] ^ x[2] ^ roundKey[r + 3];
            x[3] ^= l1(byteSub(mid)); // x[7]
        }

        // 反序变换
        for (int j = 0; j < 16; j += 4) {
            output[j] = (byte) (x[3 - j / 4] >>> 24 & 0x0ff);
            output[j + 1] = (byte) (x[3 - j / 4] >>> 16 & 0x0ff);
            output[j + 2] = (byte) (x[3 - j / 4] >>> 8 & 0x0ff);
            output[j + 3] = (byte) (x[3 - j / 4] & 0x0ff);
        }
    }

    /**
     * 密钥扩展算法
     */
    public static void sms4KeyExt(byte[] key, int[] roundKey, CryptFlag cryptFlag) {
        int[] x = new int[4];

        // 字节数组转成字数组（此处 1 字 = 32 比特）
        int[] tmp = new int[4];
        for (int i = 0; i < 4; i++) {
            tmp[0] = key[0 + 4 * i] & 0x0ff;
            tmp[1] = key[1 + 4 * i] & 0x0ff;
            tmp[2] = key[2 + 4 * i] & 0x0ff;
            tmp[3] = key[3 + 4 * i] & 0x0ff;
            x[i] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3];
        }

        // 与系统参数做异或
        x[0] ^= 0xa3b1bac6;
        x[1] ^= 0x56aa3350;
        x[2] ^= 0x677d9197;
        x[3] ^= 0xb27022dc;

        // roundKey[i] = x[i + 4] = x[i] ^ l2(byteSub(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ CK[i]))
        for (int r = 0, mid; r < 32; r += 4) {
            mid = x[1] ^ x[2] ^ x[3] ^ CK[r + 0];
            roundKey[r + 0] = x[0] ^= l2(byteSub(mid)); // x[4]

            mid = x[2] ^ x[3] ^ x[0] ^ CK[r + 1];
            roundKey[r + 1] = x[1] ^= l2(byteSub(mid)); // x[5]

            mid = x[3] ^ x[0] ^ x[1] ^ CK[r + 2];
            roundKey[r + 2] = x[2] ^= l2(byteSub(mid)); // x[6]

            mid = x[0] ^ x[1] ^ x[2] ^ CK[r + 3];
            roundKey[r + 3] = x[3] ^= l2(byteSub(mid)); // x[7]
        }

        // 解密时使用反序的轮密钥
        if (cryptFlag == CryptFlag.DECRYPT) {
            for (int r = 0, mid; r < 16; r++) {
                mid = roundKey[r];
                roundKey[r] = roundKey[31 - r];
                roundKey[31 - r] = mid;
            }
        }
    }

    public static String sm4(String inArray, String key, CryptFlag cryptFlag,
                             @Nullable Padding padding, @Nullable Mode mode, @Nullable String iv) {
        // 检查输入
        byte[] inArr = new byte[0];
        if (cryptFlag != CryptFlag.DECRYPT) {
            // 加密，输入为 utf8 串
            inArr = utf8ToArray(inArray);
        } else {
            // 解密，输入为 16 进制串
            inArr = hexToArray(inArray);
        }
        ;

        byte[] ret = sm4(inArr,
                hexToArray(key),
                cryptFlag,
                padding, mode,
                iv == null ? null : hexToArray(iv));

        // 调整输出
        if (cryptFlag != CryptFlag.DECRYPT) {
            // 加密，输出转 16 进制串
            return ArrayToHex(ret);
        } else {
            // 解密，输出转 utf8 串
            return arrayToUtf8(ret);
        }
    }

    public static byte[] sm4(byte[] inArray, byte[] key, CryptFlag cryptFlag,
                             @Nullable Padding padding, @Nullable Mode mode, @Nullable byte[] iv) {
        if (padding == null) {
            padding = Padding.PKCS_7;
        }
        if (iv == null) {
            iv = new byte[0];
        }


        if (Mode.CBC == mode) {
            // CBC 模式，默认走 ECB 模式
            if (iv.length != (128 / 8)) {
                // iv 不是 128 比特
                throw new SmException("iv is invalid");
            }
        }

        // 检查 key
        if (key.length != (128 / 8)) {
            // key 不是 128 比特
            throw new SmException("key is invalid");
        }

        // 检查输入

        // 新增填充，sm4 是 16 个字节一个分组，所以统一走到 pkcs#7
        if ((Padding.PKCS_5 == padding || Padding.PKCS_7 == padding) && cryptFlag != CryptFlag.DECRYPT) {
            int paddingCount = BLOCK - inArray.length % BLOCK;
            byte[] buff = new byte[inArray.length + paddingCount];
            System.arraycopy(inArray, 0, buff, 0, inArray.length);
            inArray = buff;
            for (int i = 0; i < paddingCount; i++) {
                buff[inArray.length + i] = (byte) paddingCount;
            }
        }

        // 生成轮密钥
        int[] roundKey = new int[ROUND];
        sms4KeyExt(key, roundKey, cryptFlag);

        List<Integer> outArray = new ArrayList<>();
        byte[] lastVector = iv;
        int restLen = inArray.length;
        int point = 0;

        while (restLen >= BLOCK) {
            byte[] input = Arrays.copyOfRange(inArray, point, point + 16);
            byte[] output = new byte[16];

            if (Mode.CBC == mode) {
                for (int i = 0; i < BLOCK; i++) {
                    if (cryptFlag != CryptFlag.DECRYPT) {
                        // 加密过程在组加密前进行异或
                        input[i] ^= lastVector[i];
                    }
                }
            }

            sms4Crypt(input, output, roundKey);


            for (int i = 0; i < BLOCK; i++) {
                if (Mode.CBC == mode) {
                    if (cryptFlag == CryptFlag.DECRYPT) {
                        // 解密过程在组解密后进行异或
                        output[i] ^= lastVector[i];
                    }
                }

//                outArray[point + i] = output[i];
                outArray.add((int) output[i]);
            }

            if (Mode.CBC == mode) {
                if (cryptFlag != CryptFlag.DECRYPT) {
                    // 使用上一次输出作为加密向量
                    lastVector = output;
                } else {
                    // 使用上一次输入作为解密向量
                    lastVector = input;
                }
            }

            restLen -= BLOCK;
            point += BLOCK;
        }

        // 去除填充，sm4 是 16 个字节一个分组，所以统一走到 pkcs#7
        if ((Padding.PKCS_5 == padding || Padding.PKCS_7 == padding) && cryptFlag == CryptFlag.DECRYPT) {
            int len = outArray.size();
            int paddingCount = outArray.get(len - 1);
            for (int i = 1; i <= paddingCount; i++) {
                if (outArray.get(len - i) != paddingCount) {
                    throw new SmException("padding is invalid");
                }
            }

//            outArray.splice(len - paddingCount, paddingCount);
            for (int i = 0; i < paddingCount; i++) {
                outArray.remove(outArray.size() - 1);
            }
        }

        // 调整输出
        byte[] ret = new byte[outArray.size()];
        for (int i = 0; i < ret.length; i++) {
            ret[i] = (byte) (int) outArray.get(i);
        }

        return ret;
    }
}
