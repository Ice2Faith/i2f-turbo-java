...
public class Sm2Cipher {
    ...
    public static String doEncrypt2(String msgStr, String publicKeyHex, CipherMode cipherMode) {
        if (cipherMode == null) {
        ...
        String c3 = CipUtils.arrayToHex(Sm3Inner.sm3(tmp));
        ...
        for (int i = 0, len = msg.length; i < len; i++) {
            ...
            // c2 = msg ^ t
            msg[i] ^= t.get()[offset.getAndIncrement()] & 0x0ff;
        }
        String c2 = CipUtils.arrayToHex(msg);

        return cipherMode == CipherMode.C1C2C3 ? c1 + c2 + c3 : c1 + c3 + c2;
    }
    ...
    public static String doDecrypt2(String encryptData, String privateKeyHex, CipherMode cipherMode) {
        if (cipherMode == null) {
            cipherMode = CipherMode.C1C3C2;
        }
        ...
        if (cipherMode == CipherMode.C1C2C3) {
            ...
            c2 = encryptData.substring(128, 128 + (encryptData.length() - 128 - 64));
        }

        byte[] msg = Utils.hexToArray(c2);
        EcPointFp c1 = Utils.getGlobalCurve().decodePointHex("04" + encryptData.substring(0, 128));
        ...
        System.arraycopy(msg, 0, tmp, x2.length, msg.length);
        System.arraycopy(y2, 0, tmp, x2.length + msg.length, y2.length);
        String checkC3 = CipUtils.arrayToHex(Sm3Inner.sm3(tmp));

        if (checkC3.equalsIgnoreCase(c3)) {
            return Utils.arrayToUtf8(msg);
        }
