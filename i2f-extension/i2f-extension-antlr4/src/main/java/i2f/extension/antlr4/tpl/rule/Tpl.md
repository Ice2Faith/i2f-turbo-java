...
```java
// 方式1：渲染模板字符串（使用默认 DefaultTplResolver）
Object result = Tpl.render("你好 ${name}", contextMap);

// 方式2：指定 Resolver 渲染
TplResolver resolver = new BindSqlTplResolver();
Object result = Tpl.render(sqlTemplate, params, resolver);

// 方式3：从文件加载模板渲染
Object result = Tpl.render(new File("template.tpl"), contextMap);

// 方式4：解析与渲染分离（复用 AST，适合高频调用同一模板）
TplParser.RootContext ast = Tpl.parse(template);
Object result1 = Tpl.render(ast, ctx1, resolver);
Object result2 = Tpl.render(ast, ctx2, resolver);
```

> `Tpl.parse()` 内部有 LRU 缓存（容量 4096），相同模板字符串只解析一次。
...
Object result = Tpl.render("用户: ${name}", user);
```

---

## DefaultTplResolver — 内置块指令

`DefaultTplResolver` 是默认的模板渲染器，直接使用 `Tpl.render(...)` 不传 resolver 时即为该实现。它内置了以下块指令。
...
## BindSqlTplResolver — 动态 SQL 构建

`BindSqlTplResolver` 继承自 `DefaultTplResolver`，专为**动态 SQL + 绑定变量**场景设计。在 Default 全部能力基础上，额外提供以下 SQL 专用特性。

### `$` vs `#` 的核心区别

这是 BindSql 模式与 Default 模式最关键的差异：

| 取值语法 | DefaultTplResolver | BindSqlTplResolver |
|---|---|---|
...
// 使用 BindSqlTplResolver 渲染
BindSqlTplResolver resolver = new BindSqlTplResolver();
Object result = Tpl.render(sqlTpl, params, resolver);
...
String tpl = "用户: ${username}, 年龄: ${age}, 状态: ${status ? \"正常\" : \"禁用\"}";
Object result = Tpl.render(tpl, map);
