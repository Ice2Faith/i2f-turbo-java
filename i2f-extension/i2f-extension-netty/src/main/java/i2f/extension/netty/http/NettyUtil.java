...
//                pipeline.addLast(httpHandler);
    ...
    public static void startNettyHttpServer(IJsonSerializer jsonSerializer, int port, String... basePackages) throws InterruptedException, IOException {
        HttpRequestDispatchAdapter httpHandler = new HttpRequestDispatchAdapter(jsonSerializer);
        httpHandler.addMappingByScanPackage(basePackages);
        ChannelInitializer<NioSocketChannel> httpInitializer = httpChannelInitializer(httpHandler);
        startNettyServer(port, httpInitializer);
    }
}
